...
from t_article ORDER BY article_id DESC LIMIT 4';
    ...
    public function get_article_comm_order(){
        $sql ='SELECT t.content ,a.*
from t_comment t ,t_article a ORDER BY post_date DESC LIMIT 4';
        return $this -> db -> query($sql) -> result();
    }

    public function get_msg_by_date(){
        $sql = 'SELECT m.*, u.username
FROM t_message m ,t_user u where u.user_id=m.user_id ORDER BY post_date desc limit 4';
        return $this -> db -> query($sql) -> result();
    }
...
WHERE article_id='.$id.' AND c.user_id=u.user_id ORDER BY comm_id DESC';
